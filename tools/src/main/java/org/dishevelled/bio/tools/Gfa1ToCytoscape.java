...
public final class Gfa1ToCytoscape implements Callable<Integer> {
    ...
    private static final String NODES_HEADER = "id\tsequence\tlength\tread_count\t" +
        "fragment_count\tkmer_count\tsequence_checksum\tsequence_uri";
    private static final String EDGES_HEADER = "source\ttarget\tinteraction\t" +
        ...
        "path_name\tordinal";
    private static final String USAGE = "dsh-gfa1-to-cytoscape -i input.gfa1.gz -n nodes.txt.gz -e edges.txt.gz";
     ...
     * @param outputNodesFile output nodes.txt file, must not be null
     * @param outputEdgesFile output edges.txt file, must not be null
     */
    public Gfa1ToCytoscape(final File inputGfa1File,
                           final File outputNodesFile,
                           final File outputEdgesFile) {
        checkNotNull(outputNodesFile);
        checkNotNull(outputEdgesFile);
    ...
    public Integer call() throws Exception {
        ...
        try {
            ...
            nodesWriter.println(NODES_HEADER);
            edgesWriter.println(EDGES_HEADER);
    ...
    public static void main(final String[] args) {
        ...
        FileArgument outputNodesFile = new FileArgument("n", "output-nodes-file", "output Cytoscape nodes.txt format file", true);
        FileArgument outputEdgesFile = new FileArgument("e", "output-edges-file", "output Cytoscape edges.txt format file", true);

        ArgumentList arguments = new ArgumentList(about, help, inputGfa1File, outputNodesFile, outputEdgesFile);
        CommandLine commandLine = new CommandLine(args);

        Gfa1ToCytoscape gfa1ToCytoscape = null;
        try {
            ...
            gfa1ToCytoscape = new Gfa1ToCytoscape(inputGfa1File.getValue(), outputNodesFile.getValue(), outputEdgesFile.getValue());
        }
        ...
        try {
            System.exit(gfa1ToCytoscape.call());
        }
        catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
