...
public final class FastaToParquet4 implements Callable<Integer> {
    ...
    private static final String USAGE = "dsh-fasta-to-parquet4 [args]";
     ...
     * @param flushAfter flush appender after each rows, must be greater than zero
     */
    public FastaToParquet4(final Path fastaPath, final File parquetFile, final String alphabet, final int rowGroupSize, final long flushAfter) {
        checkNotNull(parquetFile);
        checkArgument(rowGroupSize > 0, "row group size must be greater than zero");
        checkArgument(flushAfter > 0L, "flush after must be greater than zero");
    ...
    public static void main(final String[] args) {
        ...
        LongArgument flushAfter = new LongArgument("f", "flush-after", "flush appender after each rows, default " + DEFAULT_FLUSH_AFTER, false);

        ArgumentList arguments = new ArgumentList(about, help, fastaPath, parquetFile, alphabet, rowGroupSize, flushAfter);
        CommandLine commandLine = new CommandLine(args);

        FastaToParquet4 fastaToParquet = null;
        try
        {
            ...
            fastaToParquet = new FastaToParquet4(fastaPath.getValue(), parquetFile.getValue(), alphabet.getValue(DEFAULT_ALPHABET), rowGroupSize.getValue(DEFAULT_ROW_GROUP_SIZE), flushAfter.getValue(DEFAULT_FLUSH_AFTER));
        }
